...
def create_sample_anndata(n_cells: int = 1000, n_genes: int = 2000) -> AnnDataWrapper:
    ...
    obs_data = pd.DataFrame({
        'cell_type': np.random.choice(['T_cell', 'B_cell', 'NK_cell', 'Monocyte', 'Dendritic'], n_cells),
...
def create_individual_panel_demos(data_wrapper: AnnDataWrapper) -> None:
    ...
    umap_panel = UMAPPanel("umap_demo", embedding="X_umap", color="leiden", title="UMAP with Advanced Selection")
    ...
    violin_panel = ViolinPanel("violin_demo", gene=data_wrapper.adata.var_names[0], group_by="leiden", title="Violin Plot with Advanced Selection")
    ...
    heatmap_panel = HeatmapPanel("heatmap_demo", genes=data_wrapper.adata.var_names[:15].tolist(), title="Heatmap with Advanced Selection")
    ...
    dotplot_panel = DotPlotPanel("dotplot_demo", genes=data_wrapper.adata.var_names[:8].tolist(), group_by="leiden", title="DotPlot with Advanced Selection")
...
def create_multi_panel_comparison(data_wrapper: AnnDataWrapper) -> None:
    ...
    fig = make_subplots(
        rows=2, cols=3,
        subplot_titles=[
            "UMAP Embedding", "Violin Plot", "Heatmap",
            "QC Metrics", "DotPlot", "Selection Statistics"
        ],
        specs=[
    ...
    # DotPlot subplot
    dotplot_panel = DotPlotPanel("dotplot", genes=data_wrapper.adata.var_names[:6].tolist(), group_by="leiden")
    dotplot_panel.data_wrapper = data_wrapper
    dotplot_panel.enable_advanced_selection()
...
def create_selection_synchronization_demo(data_wrapper: AnnDataWrapper) -> None:
    ...
    fig = make_subplots(
        rows=3, cols=2,
        subplot_titles=[
            "UMAP (Primary Selection)", "Violin Plot (Linked)",
            "Heatmap (Linked)", "QC Metrics (Linked)",
            "DotPlot (Linked)", "Selection Statistics"
        ],
        specs=[
