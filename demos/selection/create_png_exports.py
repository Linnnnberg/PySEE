...
from pysee.panels.selection_types import SelectionMode, SelectionStatistics, SelectionType
...
def create_selection_demo_png(data_wrapper, output_dir: str = "selection_demo_output") -> None:
    ...
    # Add shapes
    rect = patches.Rectangle(
        (-1, -1), 2, 2, linewidth=1, edgecolor="blue", facecolor="none", linestyle=":"
    )
    ax.add_patch(rect)

    circle = patches.Circle(
        (0.5, 0.5), 1.2, linewidth=1, edgecolor="green", facecolor="none", linestyle=":"
    )
    ax.add_patch(circle)
    ...
    selections = [
        (
            "Small Rect",
            selection_manager.create_rectangular_selection((-2, -2, -1, -1), umap_coords),
        ),
        ...
        (
            "Polygon",
            selection_manager.create_polygon_selection([(-1, -1), (1, -1), (0, 1)], umap_coords),
        ),
    ]
