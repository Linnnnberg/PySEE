...
from pysee.panels.selection_types import SelectionType, SelectionMode, SelectionState, SelectionHistory, SelectionStatistics
...
def demo_selection_types(data_wrapper: AnnDataWrapper) -> Dict[str, go.Figure]:
    ...
    fig1.add_trace(go.Scatter(
        ...
        marker=dict(
            size=4,
            color=pd.Categorical(cell_types).codes,
            colorscale='viridis',
            opacity=0.7
        ),
        text=cell_types,
    ...
    fig1.add_shape(
        type="rect",
        x0=rect_bounds[0], y0=rect_bounds[1],
        x1=rect_bounds[2], y1=rect_bounds[3],
...
def demo_selection_modes(data_wrapper: AnnDataWrapper) -> go.Figure:
    ...
    for mode, (row, col) in zip(modes, positions):
        ...
        if mode in ['add', 'subtract', 'intersect']:
            ...
            fig.add_shape(
                type="circle",
                x0=0.5-1.2, y0=0.5-1.2, x1=0.5+1.2, y1=0.5+1.2,
...
def create_selection_statistics_chart(data_wrapper: AnnDataWrapper) -> go.Figure:
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=['Selection Counts', 'Selection Percentages', 'Selection by Type', 'Selection Ratios'],
    ...
    # Bar chart of counts
    fig.add_trace(
        go.Bar(x=names, y=counts, name='Selected Cells', marker_color='skyblue'),
        row=1, col=1
    )
    
    # Bar chart of percentages
    fig.add_trace(
        go.Bar(x=names, y=percentages, name='Percentage', marker_color='lightcoral'),
        row=1, col=2
    )
    ...
    fig.add_trace(
        go.Pie(labels=list(type_counts.keys()), values=list(type_counts.values()), name="Selection Types"),
        row=2, col=1
    )
    
    # Scatter plot of ratios
    fig.add_trace(
        go.Scatter(x=list(range(len(names))), y=ratios, mode='markers+lines', 
