...
class TestMemoryUsage:
    ...
    def dataset_info(self, datasets):
        ...
        return {name: DatasetFixtures.get_dataset_info(adata) for name, adata in datasets.items()}
    ...
    def test_memory_scaling(self, datasets, dataset_info):
        ...
        if small_memory > 0 and large_memory > 0:
            ...
            assert (
                scaling_factor < cell_scaling_factor * 2
    ...
    def test_memory_cleanup(self, datasets, dataset_info):
        ...
        for name, adata in datasets.items():
            ...
            assert memory_leak < 100, f"Potential memory leak for {name}: {memory_leak:.1f}MB"
