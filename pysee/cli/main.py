...
def main():
    ...
    parser.add_argument(
        "data_file",
        help="Path to the data file (h5ad, h5, or csv)"
    )
    ...
    parser.add_argument(
        "--umap-embedding",
        default="X_umap",
        help="Embedding key for UMAP plot (default: 'X_umap')"
    )
    
    parser.add_argument(
        "--umap-color",
        help="Column name to use for coloring UMAP points"
    )
    
    parser.add_argument(
        "--violin-gene",
        help="Gene name for violin plot"
    )
    
    parser.add_argument(
        "--violin-group",
        help="Column name to use for grouping violin plot"
    )
    
    parser.add_argument(
        "--export-code",
        action="store_true",
        help="Export Python code instead of running dashboard"
    )
